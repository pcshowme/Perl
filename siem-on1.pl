...
#
#   Script: seim-on.pl -- Version 2.39
...
if (grep{/loghost/} <FILE>){
	...
	print "Please inspect and manually configure this server for remote SEIM logging.\n";
	$STAT="seim-on.pl executed but it appears that this host is or had been configured for remote logging already.  Please inspect and manually configure this server for remote SEIM logging.";
	$STAT_FILE = $HNAME . ".no.SEIM";
}
else{
	print "Remote logging not previously configured, configuring now...\n\n";
	VMlocation();
	ConfigureSIEM();
	$STAT="seim-on.pl executed -- SYSLOG.CONF configured for remote SEIM logging.";
	$STAT_FILE = $HNAME . ".yes.SEIM";
...
sub ConfigureSIEM(){
	...
	#/******************************************************************************
	# Backup old syslog-ng.conf and append syslog-ng.conf with SEIM informaion
	#/******************************************************************************
...
sub VMlocation(){
	#/******************************************************************************
	# Determin if the Host is in the ADC, SDC or on site for SEIM IP configuration
	#/******************************************************************************
	
		$SEIMCFG = "204.67.120.160";     # Assumes Host is in the ADC or on-site
		@LOCADC = ( "4AV", "4AP", "AAV", "AUV" );
		@LOCSDC = ( "4SV", "4SP", "SAV", "SUV" );

		foreach $LOC (@LOCSDC) {
			if ($HNAME1UC =~ /$LOC/) {
				print "$HOST is in the SDC\n";
				$LOCATION = "ADC";
				$SEIMCFG = "204.67.202.160";     # Sets $SEIMCFG if the Host is in the SDC
			}
